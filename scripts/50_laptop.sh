...
detect_hardware() {
    ...
    if command -v dmidecode &>/dev/null; then
        ...
        if [[ -r /sys/devices/virtual/dmi/id/sys_vendor ]]; then
            vendor=$(tr '[:upper:]' '[:lower:]' < /sys/devices/virtual/dmi/id/sys_vendor | tr -d '[:space:]' || echo "unknown")
        else
            vendor="unknown"
        fi
        if [[ -r /sys/devices/virtual/dmi/id/product_name ]]; then
            model=$(tr '[:upper:]' '[:lower:]' < /sys/devices/virtual/dmi/id/product_name | tr -d '[:space:]' || echo "unknown")
        else
            model="unknown"
        fi
    fi
